...
def create_gwas_variant_id(gwas,build_version):
    ...
    if "non_effect_allele" in gwas and "effect_allele" in gwas:
        gwas["variant"] = gwas["chr"].astype(str) + "_" + gwas["pos"].astype(str) + "_" + gwas["non_effect_allele"].astype(str) + "_" + gwas["effect_allele"].astype(str)
...
def print_gwas_pvalues(date,qtl,tissue,qtl_type,trait,output_files_directory,independent_ranking=1):
    ...
    if qtl_type == "dapg_independent" or qtl_type == "conditional_independent":
        qtl_df.to_csv(output_files_directory+"QTLEnrich_significant_gwas_p_values_"+qtl_type+"_"+str(independent_ranking)+"_"+trait+"_"+tissue+"_"+str(date)+".txt", index=False, header=True,sep="\t")
    else:
        qtl_df.to_csv(output_files_directory+"QTLEnrich_significant_gwas_p_values_"+qtl_type+"_"+trait+"_"+tissue+"_"+str(date)+".txt", index=False, header=True,sep="\t")
