...
get_cis_genotype <- function(gt_df, snp_annot, coords, cis_window) {
  ...
  snp_info$varID <- paste0("X", snp_info$varID, "_", snp_info$alt_vcf)
  if (nrow(snp_info) == 0)
    return(NA)
...
main <- function(snp_annot_file, gene_annot, genotype_file, expr_df,
                 covariates_file, chrom, prefix, tissue, type, i, maf=0.05, n_folds=3, n_train_test_folds=10,
  ...
  rownames(expr_df) <- paste0(rownames(expr_df), "_", rownames(expr_df))
  samples <- rownames(expr_df)
  ...
  # Prepare output data----
  setwd(paste0('/faststorage/project/cattle_gtexs/CattleGTEx/OmiGA/TWAS/TWAS/bulk_eQTL/', tissue))
  #if (!dir.exists(celltype)) {
    #dir.create(celltype)
  #}
  #setwd(paste0('/faststorage/project/cattle_gtexs/CattleGTEx/OmiGA/TWAS/TWAS/bulk_eQTL/', tissue, '/', celltype))
  ...
  if (!dir.exists("covariances")) {
    dir.create("covariances")
  }
  setwd(paste0('/faststorage/project/cattle_gtexs/CattleGTEx/OmiGA/TWAS/TWAS/bulk_eQTL/', tissue, "/result"))
  if (!dir.exists(paste0("chr", chrom))) {
    dir.create(paste0("chr", chrom))
  }
  setwd(paste0('/faststorage/project/cattle_gtexs/CattleGTEx/OmiGA/TWAS/TWAS/bulk_eQTL/', tissue, "/weights"))
  if (!dir.exists(paste0("chr", chrom))) {
    dir.create(paste0("chr", chrom))
  }
  setwd(paste0('/faststorage/project/cattle_gtexs/CattleGTEx/OmiGA/TWAS/TWAS/bulk_eQTL/', tissue, "/covariances"))
  if (!dir.exists(paste0("chr", chrom))) {
    dir.create(paste0("chr", chrom))
  }
  
  model_summary_file <- paste0('/faststorage/project/cattle_gtexs/CattleGTEx/OmiGA/TWAS/TWAS/bulk_eQTL/', tissue, '/result/chr', chrom, '/', type, '.', tissue, '_', prefix, '_chr', chrom, '.', genes[i], '.model_summaries.txt')
  weights_file <- paste0('/faststorage/project/cattle_gtexs/CattleGTEx/OmiGA/TWAS/TWAS/bulk_eQTL/', tissue, '/weights/chr', chrom, '/', type, '.', tissue, '_', prefix, '_chr', chrom, '.', genes[i], '.weights.txt')
  tiss_chr_summ_f <- paste0('/faststorage/project/cattle_gtexs/CattleGTEx/OmiGA/TWAS/TWAS/bulk_eQTL/', tissue, '/result/chr', chrom, '/', type, '.', tissue, '_', prefix, '_chr', chrom, '_summary.txt')
  ...
  covariance_file <- paste0('/faststorage/project/cattle_gtexs/CattleGTEx/OmiGA/TWAS/TWAS/bulk_eQTL/', tissue, '/covariances/chr', chrom, '/', type, '.', tissue, '_', prefix, '_chr', chrom, '.', genes[i], '.covariances.txt')
  ...
  # Attempt to build model for each gene----
    ...
    if (all(is.na(cis_gt))) {
      ...
      if (ncol(cis_gt) >= 2) {
        ...
        rownames(adj_expression) <- paste0(rownames(adj_expression), "_", rownames(adj_expression))
        #rownames(adj_expression) <- paste0(rownames(adj_expression), "_", rownames(adj_expression))
        ...
        if (length(fit) > 0) {
          ...
          if (fit$nzero[best_lam_ind] > 0) {
            snp_annot$varID <- paste0("X", snp_annot$varID, "_", snp_annot$alt_vcf) 
