...
# (colnames: pheno,variant_id,chr,pos)
# file list of permutation results
rm -f -r ${dir_output}/nominal_combined_files.txt
nominal_combined_files=(`find ${dir_nominal} -regex ".*\.cis_qtl_pairs\.[0-9]+\.txt\.gz"`)
...
cat nominal_combined_files.txt | grep -f "/faststorage/project/farmgtex/pipeline/eQTL_tissues" | grep -v "LMM" > nominal_combined_files_filtered.txt
python3 /faststorage/project/farmgtex/pipeline/code/mashR//combine_signif_pairs_tjy.py ${dir_output}/nominal_combined_files_filtered.txt nominal_pairs -o ${dir_output}
