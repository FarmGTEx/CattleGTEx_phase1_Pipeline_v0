...
if (dropna) {
    # 丢掉所有含 NA 的行，只保留完全没有 NA 的
    zval_data.strong = zval_data.strong[rowSums(is.na(zval_data.strong)) == 0, ]
} else {
    zval_data.strong = zval_data.strong[rowSums(is.na(zval_data.strong)) <= 30, ]
}
...
zval_data.random.subset = zval_data.random.subset[,colnames(zval_data.random.subset) %in% colnames(zval_data.strong)]
