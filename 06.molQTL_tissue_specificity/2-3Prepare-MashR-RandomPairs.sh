...
cat Chr${CHR}.nominal_pairs_list_file.txt | grep -f "/faststorage/project/farmgtex/pipeline/eQTL_tissues" > Chr${CHR}.nominal_pairs_list_file_filter.txt
python3 /faststorage/project/farmgtex/pipeline/code/mashR/mashr_prepare_input.py ${dir_output}/Chr${CHR}.nominal_pairs_list_file_filter.txt nominal_pairs_Chr${CHR} -o ${dir_output} --only_zscore --dropna
