...
nFile=`cat ${dir_output}/permutation_files_filter.txt |grep -f "/faststorage/project/farmgtex/pipeline/eQTL_tissues"| wc -l`
...
do
{
    ...
    nominal_files=($(find "${dir_nominal}" -name "${name}*.cis_qtl_pairs.*.txt.gz" ! -name "*LMM*"))
    ...
    # extract_pairs
    python3  /faststorage/project/farmgtex/pipeline/code/mashR/extract_pairs_tjy.py ${dir_output}/${name}.nominal_files.txt ${dir_output}/strong_pairs.combined_signifpairs.txt.gz ${name} -o ${dir_output}
