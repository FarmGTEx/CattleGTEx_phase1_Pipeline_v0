#!/bin/bash
#--------------------------------------------------------------------------#
#              Edit Job specifications                                     #
#--------------------------------------------------------------------------#
#SBATCH -p normal              # Name of the queue
#SBATCH -N 1                       # Number of nodes(DO NOT CHANGE)
#SBATCH -n 12                       # Number of CPU cores
#SBATCH --mem=20480              # Memory in MiB(10 GiB = 10 * 1024 MiB)
#SBATCH -J rna_analysis              # Name of the job
#SBATCH --output=slurm_%x_%A.out   # STDOUT
#SBATCH --error=slurm_%x_%A.out    # STDERR
#SBATCH -t 4:00:00              # Job max time - Format = MM or MM:SS or HH:MM:SS or DD-HH or DD-HH:MM
# Create a temporary directory for the job in local storage - DO NOT CHANGE #
TMPDIR=/scratch/$USER/$SLURM_JOBID
export TMPDIR
mkdir -p $TMPDIR
#=========================================================================#
#         Your job script                                                 #
#=========================================================================#
# Replace the following with your work to be executed.
echo "Job start at $(date '+%d_%m_%y_%H_%M_%S')"
source "/home/huicongz/miniconda3/etc/profile.d/conda.sh"
      conda activate nf-farmgtex
       
      #Rscript prepare.r
      plink --vcf "/faststorage/project/farmgtex/pipeline/QTL_validation/RNA.filtered.vcf.gz" --make-bed --maf 0.05 --mac 6 --out ./imputed --cow --double-id
      awk 'BEGIN {OFS="\t"} { $2 = $1"_"$4; print $0 }' imputed.bim > temp.bim && mv temp.bim imputed.bim
      sed -i 's/_bqsr//g; s/.bqsr//g' imputed.fam
      
      
      #mkdir -p /faststorage/project/farmgtex/pipeline/QTL_validation/imputed/result
      cd /faststorage/project/farmgtex/pipeline/QTL_validation/imputed/result
      
      
      omiga --mode cis --genotype /faststorage/project/farmgtex/pipeline/QTL_validation/imputed/imputed --phenotype /faststorage/project/farmgtex/pipeline/QTL_validation/imputed/bed/rna.expr_tmm_inv.bed.gz --prefix imputed  --output-dir /faststorage/project/farmgtex/pipeline/QTL_validation/imputed/result --verbose --debug --dprop-pc-covar 0.001 --permutations 1000 --multiple-testing clipper 
      
      
      omiga --mode cis_independent --genotype /faststorage/project/farmgtex/pipeline/QTL_validation/imputed/imputed --phenotype /faststorage/project/farmgtex/pipeline/QTL_validation/imputed/bed/rna.expr_tmm_inv.bed.gz --prefix imputed --output-dir /faststorage/project/farmgtex/pipeline/QTL_validation/imputed/result --verbose --debug --dprop-pc-covar 0.001 --cis-file /faststorage/project/farmgtex/pipeline/QTL_validation/imputed/result/imputed.cis_qtl.txt.gz
      
      
      echo "Job completed at $(date '+%d_%m_%y_%H_%M_%S')"


#=========================================================================#
#         Cleanup  DO NOT REMOVE OR CHANGE                                #
#=========================================================================#
cd $SLURM_SUBMIT_DIR
rm -rf /scratch/$USER/$SLURM_JOBID