...
source "/home/huicongz/miniconda3/etc/profile.d/conda.sh"
      ...
      plink --vcf /faststorage/project/farmgtex/pipeline/QTL_validation/WGS.filtered.vcf.gz --make-bed --maf 0.05  --geno 0 --mac 6 --out ./wgs --cow --double-id
      awk 'BEGIN {OFS="\t"} { $2 = $1"_"$4; print $0 }' wgs.bim > temp.bim && mv temp.bim wgs.bim
      
      mkdir -p /faststorage/project/farmgtex/pipeline/QTL_validation/wgs/result
      cd /faststorage/project/farmgtex/pipeline/QTL_validation/wgs/result
      
      
      omiga --mode cis --genotype /faststorage/project/farmgtex/pipeline/QTL_validation/wgs/wgs --phenotype /faststorage/project/farmgtex/pipeline/QTL_validation/imputed/bed/rna.expr_tmm_inv.bed.gz --prefix wgs  --output-dir /faststorage/project/farmgtex/pipeline/QTL_validation/wgs/result --verbose --debug --dprop-pc-covar 0.001 --permutations 1000 --multiple-testing clipper 
      
      
      omiga --mode cis_independent --genotype /faststorage/project/farmgtex/pipeline/QTL_validation/wgs/wgs --phenotype /faststorage/project/farmgtex/pipeline/QTL_validation/imputed/bed/rna.expr_tmm_inv.bed.gz --prefix wgs --output-dir /faststorage/project/farmgtex/pipeline/QTL_validation/wgs/result --verbose --debug --dprop-pc-covar 0.001 --cis-file /faststorage/project/farmgtex/pipeline/QTL_validation/wgs/result/wgs.cis_qtl.txt.gz
