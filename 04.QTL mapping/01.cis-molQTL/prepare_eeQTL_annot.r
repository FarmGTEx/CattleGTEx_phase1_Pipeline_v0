...
count <- readRDS("/faststorage/project/farmgtex/pipeline/exon/exon_count_matrix.rds")

select_count <- count[,colnames(count) %in% leave]
saveRDS(select_count,paste0("/faststorage/project/farmgtex/pipeline/eeQTL/",scenario_select,"count_QTL.rds"))

write.table(colnames(select_count),paste0("/faststorage/project/farmgtex/pipeline/eeQTL/",scenario_select,"/sample_list.txt"),quote = F, sep = "\t" , col.names= F, row.names = F)